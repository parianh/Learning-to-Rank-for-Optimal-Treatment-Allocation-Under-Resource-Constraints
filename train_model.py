...
def run(n_estimators=100, max_samples=1, min_samples_leaf=1, min_samples_split=2, method=0, max_depth=np.inf, min_impurity=-100, use_ground_truth_prop=True):    
    ...
    def train_on_DR(seq=1, N=N):
        ...
        with open(f'models/autoc_forest_{seq}_{N}_{n_estimators}_{max_samples}_{min_samples_leaf}_{min_samples_split}_{min_impurity}_{max_depth}_{method}.pkl', 'wb') as f:
            pickle.dump(est, f)
    ...
    for s in tqdm(range(30)):  # Iterate over 30 seeds
        rfs_curr_val, rfs_curr_test = train_on_DR(s, N)  # s is the seed
    ...
    dct_val[N] = rfs_val
    dct_test[N] = rfs_test
    with open(f'val_results/results_{N}.pkl', 'wb') as f:
        pickle.dump(dct_val, f)
    with open(f'test_results/results_{N}.pkl', 'wb') as f:
